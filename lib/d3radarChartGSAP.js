...
export function RadarChart(id, data, options, questionToPillar, pillarColors) {
  ...
  const rotationDuration = 1; // Duration for one full sweep (was 2 seconds)
  ...
  tl.to(
    ...
    "-=0.25" // Overlap the start of the sweep with the circles animation
  );
  ...
  tl.to(sweepLine.node(), { duration: 0.25, opacity: 0 });
