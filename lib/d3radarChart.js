...
export function RadarChart(id, data, options) {
  ...
  const radarLine = d3
    ...
    .angle((d, i) => {
      // Plot on every other axis starting from the first
      return angleSlice * (i * 2) - Math.PI / 2;
    })
  ...
  // Append the circles on visible axes
  ...
  blobWrapper
    ...
    .attr(
      "cx",
      (d, i) => rScale(d.value) * Math.cos(angleSlice * (i * 2) - Math.PI / 2)
    )
    .attr(
      "cy",
      (d, i) => rScale(d.value) * Math.sin(angleSlice * (i * 2) - Math.PI / 2)
    )
  ...
  if (cfg.tooltip) {
    ...
    // Append invisible circles on visible axes
    blobWrapper
